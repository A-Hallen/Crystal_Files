...
class PinchRecyclerView : RecyclerView {
    ...
    private inner class ScaleListener : SimpleOnScaleGestureListener() {
        override fun onScale(detector: ScaleGestureDetector): Boolean {
            ...
            println("HALLEN mScaleFactor: $mScaleFactor")
            scale = prefs.saveScala(((mScaleFactor * 2).toInt() - 6) * -1 + 2)
            (this@PinchRecyclerView.layoutManager as GridLayoutManager).spanCount = scale
            //(this@PinchRecyclerView.layoutManager as GridLayoutManager).setMeasuredDimension()
            invalidate()
            return true
        }
